...
namespace Lean.Gui
{
	...
	public class LeanSwitch : MonoBehaviour
	{
		...
		/// <summary>This allows you to switch to the next state without needing to reference state by index</summary>

		public void Next()
        {
			Switch((state == states.Count - 1) ? 0 : state + 1);
        }

		/// <summary>This allows you to switch to the previous state without needing to reference state by index</summary>

		public void Previous()
        {
			Switch((state == 0) ? states.Count - 1 : state - 1);
        }
