...
namespace ThreeEyedGames
{
    ...
    public class DecaliciousRenderer : MonoBehaviour
    {
...
#endif
        public static DecaliciousRenderer singleton;
        ...
        protected static Material _materialLimitToGameObjects;
        protected static Vector4[] _avCoeff = new Vector4[7];
        private static Shader limitShader;
        protected const float _noLimitToValue = 0.0f;

        private static readonly int limitToId = Shader.PropertyToID("_DecaliciousLimitToGameObject");
        private static readonly int idHash = Shader.PropertyToID("_ID");
        ...
        private void Awake()
        {
            if(Application.isPlaying && singleton != null)
            {
                Debug.LogWarning("Decal Renderer: Multiple singletons detected, investigate.");
                Destroy(this);
                return;
            }
            singleton = this;

            limitShader = Shader.Find("Hidden/Decalicious Game Object ID");
        }
        ...
        private void DrawLimitToGameObjects(Camera cam)
        {
            ...
            if (_materialLimitToGameObjects == null)
                _materialLimitToGameObjects = new Material(limitShader);
            ...
            {
                _bufferLimitTo.SetGlobalFloat(idHash, go.GetInstanceID());
        ...
        private static readonly int unity_SHAg = Shader.PropertyToID("unity_SHAg");
        private static readonly int unity_SHAb = Shader.PropertyToID("unity_SHAb");
        private static readonly int unity_SHBr = Shader.PropertyToID("unity_SHBr");
        private static readonly int unity_SHBg = Shader.PropertyToID("unity_SHBg");
        private static readonly int unity_SHBb = Shader.PropertyToID("unity_SHBb");
        private static readonly int unity_SHC = Shader.PropertyToID("unity_SHC");

        private static readonly int _MaskMultiplier = Shader.PropertyToID("_MaskMultiplier");
        private static readonly int _LimitTo = Shader.PropertyToID("_LimitTo");
        ...
        private void DrawDeferredDecals_Albedo(Camera cam)
        {
            ...
            while (allRenderOrderEnum.MoveNext())
            {
                ...
                allMaterialEnum.Dispose();
            }

            allRenderOrderEnum.Dispose();
        }

        private static readonly int copy1id = Shader.PropertyToID("_CameraGBufferTexture1Copy");
        private static readonly int copy2id = Shader.PropertyToID("_CameraGBufferTexture2Copy");
        ...
        {
            if (!_deferredDecals.TryGetValue(decal.RenderOrder, out Dictionary<Material, HashSet<Decal>> dict))
            {
                dict = new Dictionary<Material, HashSet<Decal>>();
                _deferredDecals.Add(decal.RenderOrder, dict);
            }

            if(dict.TryGetValue(decal.Material, out HashSet<Decal> value))
            {
                value.Add(decal);
            }
            else
        ...
        {
            if (!_unlitDecals.TryGetValue(decal.RenderOrder, out Dictionary<Material, HashSet<Decal>> dict))
            {
                dict = new Dictionary<Material, HashSet<Decal>>();
                _unlitDecals.Add(decal.RenderOrder, dict);
            }
