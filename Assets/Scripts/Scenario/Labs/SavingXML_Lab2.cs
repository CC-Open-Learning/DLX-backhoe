 ...
 *  DESCRIPTION   : This file saves the contents of the menu for the Lab #2, including GA1,GA2,and comments.
 ...
 */
public class SaveFile_Lab2
{
    [XmlArrayItem(Type = typeof(Result_Lab2))]
    public Result_Lab2[] Results;//breaking naming convention for array Results for easier XML reading. 
}
/*
 * CLASS:       Result
 ...
 */
public class Result_Lab2
{
    public string GA1FieldString;
    public string GA2FieldString;
    public string commentFieldString;
}
 ...
 */
public class SavingXML_Lab2 : MonoBehaviour
{
    ...
    public GameObject[] GA1FieldOBJ = new GameObject[7];
    public GameObject[] GA2FieldOBJ = new GameObject[7];
    public GameObject[] commentFieldOBJ = new GameObject[7];
    /*
     * FUNCTION    : SaveAsXML()
    ...
    {
        SaveFile_Lab2 save = new SaveFile_Lab2();
        Result_Lab2[] resultArray = new Result_Lab2[7];
        for (int j = 0; j < SIZE; j++)
        {
            resultArray[j] = new Result_Lab2(); 
        }
        

        for (int i = 0; i < SIZE; i++)
        {
            string text = GA1FieldOBJ[i].GetComponent<Text>().text;
            resultArray[i].GA1FieldString = text;
            //Debug.Log(save.Results[i].GA1FieldString);

            text = GA2FieldOBJ[i].GetComponent<Text>().text;
            resultArray[i].GA2FieldString = text;
            //Debug.Log(save.Results[i].GA2FieldString);

            text = commentFieldOBJ[i].GetComponent<Text>().text;
            resultArray[i].commentFieldString = text;
            //Debug.Log(save.Results[i].commentFieldString);
        }

        save.Results = resultArray;

        XmlSerializer s = new XmlSerializer(typeof(SaveFile_Lab2));

        var path = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "//SaveFile_Lab2.xml";
